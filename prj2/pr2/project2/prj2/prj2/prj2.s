...
ti_inthandler:
	!FIX ME
	la $a0, savek0			! load address of savek0
	sw $k0, 0($a0)			! save k0
	ei						! enable interrupt
	la $a0, saves0			! load address of saves0
	sw $s0, 0($a0)			! save s0
	sw $s1, 1($a0)			! save s1
	sw $s2, 2($a0)			! save s2
	sw $a0, 3($a0)			! save a0
...
wb: 
	...
	la $a0, saves0			! load address of saves0
	lw $s0, 0($a0)			! restore $s0
	lw $s1, 1($a0)			! restore $s1
	lw $s2, 2($a0)			! restore $s2
	lw $a1, 4($a0)			! restore $a1
	lw $a0, 3($a0)			! restore $a0
	di						! disable interrupt
	la $a0, savek0			! load address of savek0
	lw $k0, 0($a0)			! restore $k0
	reti					! return from interrupt
	savek0:	.fill 0x0		! save k0
	saves0: .fill 0x0		! save s0
	saves1:	.fill 0x0		! save s1
	saves2:	.fill 0x0		! save s2
	savea0: .fill 0x0		! save a0
	savea1:	.fill 0x0		! save a1
